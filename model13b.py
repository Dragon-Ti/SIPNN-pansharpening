...
class SIPNN(nn.Module):
    def __init__(self, res_block_num=4, block_num=4, spectral_num=8):
        ...
        self.deconv2 = nn.ConvTranspose2d(in_channels=spectral_num, out_channels=spectral_num, kernel_size=8, stride=4,
                                          padding=2, bias=True)
        self.conv1_lms = nn.Conv2d(in_channels=spectral_num + 1, out_channels=64, kernel_size=3, padding=1, bias=True)
    ...
    def forward(self, ms, pan):
        lms = self.deconv1(ms)
        lms_feature = self.relu(self.conv1_lms(torch.cat((lms, pan), dim=1)))
