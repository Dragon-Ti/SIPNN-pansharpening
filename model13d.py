...
# 改进：每个RSIFFBlock加入SEModule
# 改进：把PAN_HP的卷积增加到4
# 增加deconv，ms用于训练而不是lms
# SPITAL 从4改8,网络改成resblock
# 最后输出前直接加lms的结果
# RSIFFBlock中卷积改为2，block个数减半，先SE再加法，同时加spatial和残差
...
class RSIFF_block(nn.Module):
    def __init__(self):
        super(RSIFF_block, self).__init__()
    ...
    def forward(self, lms, spatial_info):
        output = self.se(lms, self.conv2(self.relu(self.conv1(lms)))) + spatial_info + lms
        return output
...
class RSIFF(nn.Module):
    def __init__(self, resblock_num=4, block_num=4, spectral_num=8):
        super(RSIFF, self).__init__()
        self.SIPNN_backbone = nn.ModuleList([RSIFF_block() for i in range(block_num)])
        ...
        self.spatial = SPATIAL_Branch(spectral_num, resblock_num)
